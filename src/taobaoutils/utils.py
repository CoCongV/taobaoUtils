...
def send_request(target_url, payload, logger, config_data):
    """
    向目标 URL 发送 POST 请求。
...
def load_excel_data(config_data, logger):
    """
...
def save_dataframe(df, index, config_data, logger):
    """
...
def process_row(df, index, row, last_send_time, config_data, logger):
    """
    处理 DataFrame 中的单行数据。
    :param df: pandas.DataFrame
    :param index: 当前行索引
    ...
    success, response_text = send_request(
        config_data['TARGET_URL'], current_payload, logger, config_data)
